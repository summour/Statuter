...
export const DeckReader: React.FC<DeckReaderProps> = ({
  ...
  const [structureFilter, setStructureFilter] = useState<string>(initialStructureFilter);
  const [isTocModalOpen, setIsTocModalOpen] = useState<boolean>(false);

  // Sync initial props if changed
  useEffect(() => {
    if (initialStructureFilter) {
      setStructureFilter(initialStructureFilter);
    }
  ...
  return (
    <div className="max-w-4xl mx-auto px-3 sm:px-6 py-3 sm:py-6">
      ...
      <div className="bg-white rounded-2xl p-2.5 sm:p-3.5 border border-zinc-200 shadow-2xs mb-3 sm:mb-4 flex items-center justify-between gap-2">
        {/* Left: Back to Overview & Section Progress */}
        <div className="flex items-center gap-2 min-w-0">
          <button
            id="back-to-library-btn"
            onClick={onBackToLibrary}
            className="flex items-center gap-1 px-2.5 py-1.5 rounded-xl bg-zinc-100 hover:bg-zinc-200 active:bg-zinc-300 text-zinc-800 text-xs font-semibold transition-colors cursor-pointer shrink-0"
            title="กลับสู่หน้าสารบัญและโครงสร้าง"
          >
          ...
          <div className="flex items-center gap-1.5 min-w-0">
            <span className="text-[11px] font-bold px-2 py-0.5 rounded-md bg-zinc-900 text-white shrink-0">
              {deckShort}
            </span>
            <span className="text-xs font-medium text-zinc-600 truncate hidden xs:inline">
              {formatNumeralText(`ม. ${currentIndex + 1}/${deckCards.length}`, numeralSystem)}
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          ...
          <div className="relative">
            <select
              ...
              className="appearance-none bg-zinc-100 hover:bg-zinc-200 border border-zinc-200 text-xs font-semibold text-zinc-800 py-1.5 pl-2.5 pr-6 rounded-xl cursor-pointer focus:outline-none max-w-[105px] sm:max-w-[140px] truncate"
            >
          ...
          <div className="flex items-center bg-zinc-100 rounded-xl p-0.5 border border-zinc-200">
            <button
              ...
              className={`p-1.5 rounded-lg text-xs font-medium transition-all cursor-pointer ${
                viewMode === 'card' ? 'bg-white text-zinc-900 shadow-2xs' : 'text-zinc-500 hover:text-zinc-900'
              }`}
              title="ดูแบบการ์ด"
            >
          ...
          {onNumeralSystemChange && (
            <div 
              className="hidden xs:flex items-center bg-zinc-100 rounded-xl p-0.5 border border-zinc-200"
              title={`ระบบตัวเลข: ${numeralSystem === 'arabic' ? 'เลขอารบิก' : 'เลขไทย'}`}
            >
              <button
          ...
          <button
            ...
            className="px-2 py-1 bg-zinc-100 hover:bg-zinc-200 text-zinc-700 rounded-xl text-xs font-bold transition-colors cursor-pointer"
            title="ขนาดตัวอักษร"
          >
      ...
      {structureFilter !== 'all' && (
        <div className="flex items-center justify-between bg-zinc-50 border border-zinc-200 px-3.5 py-2 rounded-xl text-xs text-zinc-700 mb-3">
          <div className="flex items-center gap-1.5 min-w-0">
            <span className="font-semibold text-zinc-900 shrink-0">หมวด:</span>
            ...
            <span className="text-[11px] text-zinc-400 shrink-0">
              ({formatNumeralText(deckCards.length.toString(), numeralSystem)} ม.)
            </span>
          </div>
          <div className="flex items-center gap-2 shrink-0 ml-2">
            ...
            <button
              ...
              className="text-[11px] font-semibold text-zinc-400 hover:text-zinc-900 cursor-pointer"
              title="ล้างตัวกรองและดูทั้งหมด"
            >
              ✕
            </button>
          </div>
